"""Visualizes what embeddings/service.py's GaitRecognitionService.embed_video
actually does to a clip — the pipeline (YOLO+DeepSORT track -> MOG2
background subtraction -> contour cleanup -> Gait Energy Image -> CNN
embedding) otherwise only ever produces a numeric vector, no visible proof of
any intermediate step. This draws all three OpenCV stages live, every frame:
...
from embeddings.gei import normalize_silhouette
from embeddings.silhouette import MIN_SILHOUETTE_AREA
from inference.pipeline import Track
...
def _draw_overlay(
    ...
    cv2.putText(
        out, "YOLO+DeepSORT track", (x1, max(y1 - 8, 16)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 0), 2, cv2.LINE_AA
    )
...
def visualize_gait_extraction(track: Track, output_path: Path, fps: float = 24.0) -> dict:
    ...
    for frame, bbox, crop, (raw_mask, clean_mask) in zip(track.frames, track.bboxes, crops, stages, strict=False):
        normalized = normalize_silhouette(clean_mask)
    ...
    return {
        ...
        "gei_captured": gei_captured,
        "silhouette_frames_used": running_count,
    }

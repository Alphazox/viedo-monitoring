...
def track_centroids(video_path: str) -> Trajectory:
    """Real YOLO+DeepSORT tracking first (app/detection/) — a track's bbox
    centers are a direct, more accurate position trail than deriving one
    from a silhouette mask. Falls back to the legacy whole-frame background-
    subtraction + silhouette-moments approach only if no person was detected
    — e.g. this repo's own synthetic demo clips (see app/detection/__init__.py).
    """

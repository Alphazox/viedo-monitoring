...
# and alerts_store.py's alert_log) so the YOLO weights download happens once
# at startup, not on the first request.
_detector = PersonDetector(Path(_settings.demo_data_dir) / "detection")
...
def track_people(video_path: str, max_frames: int = MAX_FRAMES) -> list[Track]:
    ...
    app/detection/__init__.py for why that's expected for this repo's own

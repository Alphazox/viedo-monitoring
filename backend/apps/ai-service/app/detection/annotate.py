"""Draws real YOLOv8+DeepSORT detections onto a clip's frames — the
pipeline in pipeline.py/tracker.py already does the detection and tracking;
nothing before this consumed it as a visible overlay, only as numeric
tracks/bboxes fed into gait embedding or the activity heuristic.
"""
...
from app.detection.tracker import PersonTracker
...
def annotate_video(detector: PersonDetector, input_path: str, output_path: Path, max_frames: int = MAX_FRAMES) -> dict:
    ...
    while frame_count < max_frames:
        ...
        for track_id, (x1, y1, x2, y2) in active:
            ...
            cv2.putText(annotated, label, (x1, label_y), cv2.FONT_HERSHEY_SIMPLEX, 0.55, BOX_COLOR, 2, cv2.LINE_AA)
    ...
    return {
        ...
        "tracksFound": len(track_ids_seen),
        "maxDetectionsInFrame": max_detections_in_frame,
    }

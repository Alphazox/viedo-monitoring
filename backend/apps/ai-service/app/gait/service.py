...
from app.gait.gei import build_gei
from app.gait.model import GaitEmbeddingNet
from app.gait.silhouette import extract_silhouettes, read_video_frames

logger = logging.getLogger(__name__)

# Fixed seed so a fresh deployment (no saved weights yet) is reproducible run
# to run — see app/gait/__init__.py for what this untrained model does and
# doesn't prove.
_INIT_SEED = 1337
...
class GaitRecognitionService:
    def __init__(self, weights_path: Path) -> None:
        if weights_path.exists():
            ...
            logger.info("Initialized new gait embedding weights at %s (seed=%d)", weights_path, _INIT_SEED)

        model.eval()
        self._model = model

    def embed_video(self, video_path: str) -> np.ndarray:
        # Real YOLO+DeepSORT detection/tracking first (app/detection/); falls
        # back to the legacy whole-frame background-subtraction heuristic
        # only if no person was detected — e.g. this repo's own synthetic
        # demo clips, which don't look like people to a real detector (see
